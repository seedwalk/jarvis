...
Example: `/jarvis-insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /jarvis-insert-phase <after> <description>"
  echo "Example: /jarvis-insert-phase 72 Fix critical auth bug"
  exit 1
fi
```
...
2. Insert new phase heading with (INSERTED) marker:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /jarvis-plan-phase {decimal_phase} to break down)
...
`/jarvis-plan-phase {decimal_phase}`
...
- Don't use this for planned work at end of milestone (use /jarvis-add-phase)
