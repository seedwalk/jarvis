...
**After this command:** Run `/jarvis-plan-phase 1` to start execution.
...
1. **Abort if project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] && echo "ERROR: Project already initialized. Use /jarvis-progress" && exit 1
   ```
...
- options:
  - "Map codebase first" — Run /jarvis-map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/jarvis-map-codebase` first, then return to `/jarvis-new-project`
```
Exit command.
...
Create `opencode.json` in the project root. This is a derived config that assigns a model to each Jarvis agent.
...
**Important:** OpenCode loads `opencode.json` at session start and does not hot-reload. If you change profiles later via `/jarvis-set-profile` or `/jarvis-settings`, run `/new` (or restart OpenCode) for it to take effect.

**Note:** Run `/jarvis-settings` anytime to update these preferences.
...
/jarvis-discuss-phase 1 — gather context and clarify approach
...
**Also available:**
- /jarvis-plan-phase 1 — skip discussion, plan directly
...
- [ ] User knows next step is `/jarvis-discuss-phase 1`
