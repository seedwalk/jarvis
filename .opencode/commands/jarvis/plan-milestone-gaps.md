---
name: jarvis-plan-milestone-gaps
...
<objective>
Create all phases necessary to close gaps identified by `/jarvis-audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/jarvis-add-phase` per gap.
</objective>
...
```
No audit gaps found. Run `/jarvis-audit-milestone` first.
```
...
**Also available:**
- `/jarvis-execute-phase {N}` — if plans already exist
...
`/jarvis-audit-milestone` — re-audit to verify gaps closed
`/jarvis-complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/jarvis-plan-phase` next
</success_criteria>
