---
name: jarvis-quick
description: Execute a quick task with Jarvis guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
tools:
  - read
  - write
...
<objective>
Execute small, ad-hoc tasks with Jarvis guarantees (atomic commits, STATE.md tracking) while skipping optional agents (research, plan-checker, verifier).
...
<execution_context>
Orchestration is inline - no separate workflow file. Quick mode is deliberately simpler than full Jarvis.
</execution_context>
...
Check that an active Jarvis project exists:

```bash
if [ ! -f .planning/ROADMAP.md ]; then
  echo "Quick mode requires an active project with ROADMAP.md."
  echo "Run /jarvis-new-project first."
  exit 1
fi
```
...
Ready for next task: /jarvis-quick
```
