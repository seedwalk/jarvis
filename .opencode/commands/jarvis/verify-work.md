...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /jarvis-execute-phase
</objective>
...
8. If issues found:
   ...
   - Present ready status with `/new` then `/jarvis-execute-phase`
</process>
...
- /jarvis-execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- /jarvis-complete-milestone — skip audit, archive directly
...
/jarvis-execute-phase {Z} --gaps-only
...
- /jarvis-plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /jarvis-plan-phase {Z} --gaps — retry fix planning with guidance
- /jarvis-discuss-phase {Z} — gather more context before replanning
...
- [ ] Ready for `/jarvis-execute-phase` when complete
</success_criteria>
